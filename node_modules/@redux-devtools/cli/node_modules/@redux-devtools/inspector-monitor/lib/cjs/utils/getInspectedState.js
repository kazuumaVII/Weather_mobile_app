...
function getInspectedState(state, path, convertImmutable) {
  state = path && path.length ? (0, _defineProperty2.default)({}, path[path.length - 1], path.reduce(function (s, key) {
    if (!s) {
      return s;
    }

    if ((0, _immutable.isAssociative)(s)) {
      return s.get(key);
    } else if ((0, _isIterable.default)(s)) {
