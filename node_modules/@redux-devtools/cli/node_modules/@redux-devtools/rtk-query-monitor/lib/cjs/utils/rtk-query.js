...
function computeRtkQueryRequests(type, api, sortedActions, currentStateIndex) {
  ...
  for (var _i2 = 0, _len2 = queryCacheEntries.length; _i2 < _len2; _i2++) {
    ...
    var _queryCacheEntries$_i = (0, _slicedToArray2.default)(queryCacheEntries[_i2], 2),
...
function computeQueryApiTimings(requestById) {
  ...
  var average = pendingDurations.length > 0 ? (0, _formatters.formatMs)(statistics.mean(pendingDurations)) : '-';
  var median = pendingDurations.length > 0 ? (0, _formatters.formatMs)(statistics.median(pendingDurations)) : '-';
  return {
...
function computeApiTimings(api, actionsById, currentStateIndex) {
  var sortedActions = Object.entries(actionsById).sort(function (thisAction, thatAction) {
    return (0, _comparators.compareJSONPrimitive)(Number(thisAction[0]), Number(thatAction[0]));
  }).map(function (entry) {
