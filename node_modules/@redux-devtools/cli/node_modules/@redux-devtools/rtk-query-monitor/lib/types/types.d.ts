...
export declare enum QueryPreviewTabs {
    queryinfo = 0,
    apiConfig = 1,
    querySubscriptions = 2,
    queryTags = 3,
    actions = 4
}
...
export interface RtkQueryMonitorProps<S, A extends Action<unknown>> extends LiftedState<S, A, RtkQueryMonitorState> {
    dispatch: Dispatch<Action | LiftedAction<S, A, RtkQueryMonitorState>>;
    theme: keyof typeof themes | Base16Theme;
    invertTheme?: boolean;
}
export declare type RtkQueryApiState = ReturnType<ReturnType<typeof createApi>['reducer']>;
export declare type RtkQueryState = NonNullable<RtkQueryApiState['queries'][keyof RtkQueryApiState]>;
export declare type RtkMutationState = NonNullable<RtkQueryApiState['mutations'][keyof RtkQueryApiState]>;
export declare type RtkQueryApiConfig = RtkQueryApiState['config'];
export declare type RtkQueryProvided = RtkQueryApiState['provided'];
...
export declare type RtkResourceInfo = QueryInfo | MutationInfo;
export interface ApiInfo {
    reducerPath: string;
    apiState: RtkQueryApiState;
}
export interface SelectOption<T = string, VisConfig extends string = 'default'> {
    label: string;
    value: T;
...
export declare type RTKQuerySubscribers = NonNullable<RtkQueryApiState['subscriptions'][keyof RtkQueryApiState['subscriptions']]>;
...
export declare type QueryTally = {
...
export interface TabOption<S, P, V extends string = 'default'> extends SelectOption<S, V> {
...
export declare type RtkRequest = {
    status: QueryStatus;
    queryKey: string;
    requestId: string;
    endpointName: string;
