...
export default function DayOther({ dataBase }) {
	return (
		<View style={styles.container}>
			<BlurView intensity={70} style={styles.blurDiv}>
				<ScrollView horizontal={true} showsHorizontalScrollIndicator={false} alwaysBounceHorizonta={true} scrollEnabled={true}>
					{dataBase?.daily.map((data, index) => (
						<View key={index}>{DayBloc(index, data.temp.max, data.temp.min, data.weather[0].icon)}</View>
					))}
				</ScrollView>
			</BlurView>
		</View>
